...
@implementation ScanDBOperation
...
- (void)main
{
   // firstTime = true;
    AppDelegate* delegate = [[UIApplication sharedApplication] delegate];
    NSManagedObjectContext* localScanDBContext = [[NSManagedObjectContext alloc] initWithConcurrencyType:NSPrivateQueueConcurrencyType];
    
    NSPersistentStoreCoordinator* psc = [delegate persistentStoreCoordinator];
    [localScanDBContext setPersistentStoreCoordinator:psc];
    [localScanDBContext setUndoManager:nil];
    
    NSNotificationCenter* center = [NSNotificationCenter defaultCenter];
    [center addObserver:self selector:@selector(mergeChanges:) name:NSManagedObjectContextDidSaveNotification object:localScanDBContext];
    
    [localScanDBContext performBlockAndWait:^{
        
        //Perform Fetch from DB
        
        NSFetchRequest* request = [[NSFetchRequest alloc] init];
        NSEntityDescription* entity2 = (NSEntityDescription*)[NSEntityDescription entityForName:@"GenProfile" inManagedObjectContext:localScanDBContext];
        ...
        NSError* error;
        NSArray* fetchedObjects = [localScanDBContext executeFetchRequest:request error:&error];
        
        authorizationString = [QKCoreDataInterface getAuthorizationString:@"" :localScanDBContext];
    ...
    [localScanDBContext setParentContext:moc];
    
    [localScanDBContext performBlock:^{
        
        
        NSFetchRequest* request = [[NSFetchRequest alloc] init];
        NSEntityDescription* entity2 = (NSEntityDescription*)[NSEntityDescription entityForName:@"Skills" inManagedObjectContext:localScanDBContext];
        ...
        if (iCount % 50 == 0) {
            ...
            if (! [localScanDBContext save:&error]) {
                NSLog(@"Error saving context: %@", [error localizedDescription]);
            }
        }
    }];
...
- (void) callProfileFillingService : (GenProfile*)genProfileObject
{
   ...
   // NSString* accessToken = [dic valueForKey:@"access_token"];
    ...
    request.authorizationString = authorizationString;
   ...
   //     NSLog(@"%@",result);
        ...
        if([number isEqualToNumber:[NSNumber numberWithInt:1]]) //Profile Filling Update Success
        {
            ...
            responseMessage = @"UPDATED PROFILE TO SERVER SUCESSFULLY";
...
- (void)mergeChanges:(NSNotification *)notification
{
    ...
    [managedObjectContext performSelectorOnMainThread:@selector(mergeChangesFromContextDidSaveNotification:) withObject:notification waitUntilDone:NO];
